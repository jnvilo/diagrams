...
def _format_info(info):
    ...
    """
    wrapper = textwrap.TextWrapper(width=40, max_lines=3)
    lines = [html.escape(line) for line in wrapper.wrap(info)]
    lines += [""] * (3 - len(lines))  # fill up with empty lines so it is always three
    return "<br/>".join(lines)


def VirtualNetwork(name, vnet="", resource_group="", type="Container", **kwargs):
    key = f"{vnet}: {vnet}" if vnet else type
    description = f"{resource_group}"
    node_attributes = {
        "label": _format_node_label(name, f"vnet: {vnet}", description),
        "labelloc": "c",
        "shape": "rect",
        "width": "3",
        "height": "1.6",
...
def AbakusServicesGroups(name, vnet="", resource_group="", **kwargs):
    ...
    return VirtualNetwork(**container_attributes)


# def ABakusServicesGroup(name, )
def AzureServicesGroup(name)
...
def Container(name, technology="", description="", **kwargs):
    ...
    container_attributes.update(kwargs)
    return AbakusNode(**container_attributes)
